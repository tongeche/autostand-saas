...
import { assembleDocContext, resolvePath } from "../services/context.js";

// Simple {{ key }} placeholder renderer (supports dot paths like lead.name)
function render(body, ctx){
  const C = ctx || {};
  return String(body||"").replace(/{{\s*([\w.]+)\s*}}/g, (_,k)=>{
    const v = resolvePath(C, k);
...
export default function DocumentManager(){
  ...
  async function openEditorFromTemplate(tpl, lead){
    try{
      setBusy(true);
      const ctx = await assembleDocContext({ leadId: lead?.id });
...
function printableHtml(title, content, ctx, docType){
  ...
  const assetsBlock = (()=>{
    const imgs = Array.isArray(ctx?.assets) ? ctx.assets.filter(a => a.url) : [];
    if (!imgs.length) return '';
    const items = imgs.slice(0,6).map(a => `<img src=\"${a.url}\" alt=\"${(a.title||'').replace(/\"/g,'&quot;')}\" style=\"width:100%;height:120px;object-fit:cover;border-radius:8px\"/>`).join('');
    return `<div style=\"margin-top:16px\">\n      <div style=\"font-weight:600\">Imagens</div>\n      <div style=\"display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px\">${items}</div>\n    </div>`;
  })();
