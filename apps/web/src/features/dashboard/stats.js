...
export async function fetchStats() {
  ...
  // 1) total leads
  const totalLeads = await safeCount("leads", (q) => q.eq("tenant_id", tenantId));

  // 2) new leads (last 30 days) — requires leads.created_at
  const since30 = new Date();
  since30.setDate(since30.getDate() - 30);
  const newLeads = await safeCount("leads", (q) =>
    q.eq("tenant_id", tenantId).gte("created_at", since30.toISOString())
  );

  // 3) inventory items
  const inventory = await safeCount("gallery_items", (q) => q.eq("tenant_id", tenantId));

  // 4) activities in last 7 days — requires audit_logs.created_at
  const since7 = new Date();
  since7.setDate(since7.getDate() - 7);
  const activities7d = await safeCount("audit_logs", (q) =>
    q.eq("tenant_id", tenantId).gte("created_at", since7.toISOString())
  );

  // 5) ACTIVE LEADS (exclude closed outcomes like won/lost/archived)
  const CLOSED = '("won","lost","archived")'; // adjust if your statuses differ
  const activeLeads = await safeCount("leads", (q) =>
    q.eq("tenant_id", tenantId).not("status", "in", CLOSED)
  );
